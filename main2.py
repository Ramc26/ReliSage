...
class RepoAnalyzer:
    ...
    def _parse_repo_url(self):
        ...
        if self.provider == "github":
            ...
            # Handle GitLab's project path (which might include subgroups)
            project_path = path
    ...
    def _get_github_commits(self, max_commits):
        ...
        for commit in response.json():
            ...
            formatted_files = [f"{f['filename']} ({f['changes']} changes)" 
    ...
    def _get_gitlab_commits(self, max_commits):
        ...
        for commit in response.json():
            ...
            commits.append({
                ...
                "files": [f"{f['new_path']} ({self._gitlab_change_type(f)})" 
                        for f in commit_details['diff']],
...
def generate_release_notes(commits):
    ...
    prompt = f"""Analyze these Git commits and generate professional release notes. Focus on:
